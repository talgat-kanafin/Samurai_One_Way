...
from core.database import (
    ...
    update_idea_entry_color, update_idea_entry_phase
)
from core.models import ChatSession, EntryColor
from ai.client import send_message


PROMPT_MAP = {
    "software":   "idea_prog",
    "production": "idea_prod",
    "research":   "research",
}
...
class ChatScreen(QWidget):
    confirmed = Signal(str)  # entry_id — когда пользователь подтвердил

    def __init__(self, entry_id: str, theme: str, description: str, main_window):
        super().__init__()
        self.entry_id = entry_id
        self.theme = theme
        self.description = description
        self.main_window = main_window
        self.prompt_id = PROMPT_MAP.get(theme, "idea_prog")
        self.phase = 0
        self.messages = []
    ...
    def _build_ui(self):
        ...
        self.btn_revision.setStyleSheet("""
            ...
            QPushButton:hover { background-color: #9a7400; }
            QPushButton:disabled { background-color: #333; color: #666; }
        """)
    ...
    def _load_or_start(self):
        ...
        if row:
            ...
            # Первый запуск — автоматически отправляем описание
            self._start_phase_1()

    def _start_phase_1(self):
        self.phase = 1
        update_idea_entry_color(self.entry_id, EntryColor.YELLOW.value)
        update_idea_entry_phase(self.entry_id, 1)
        self._add_bubble(self.description, is_user=True)
        self.messages.append({"role": "user", "text": self.description})
        self._save_session()
        self._call_ai(self.description)
    ...
    def _on_revision(self):
        ...
        text = self.input.toPlainText().strip()
        if not text:
            text = "Прошу доработать финальный результат"
        self.input.clear()
        self.messages.append({"role": "user", "text": text})
        self._add_bubble(text, is_user=True)
        self._save_session()
...
class MessageBubble(QFrame):
    def __init__(self, text: str, is_user: bool, is_error: bool = False):
        super().__init__()
        ...
        if is_error:
            bg, color = "#3a1a1a", "#ff6b6b"
        elif is_user:
            bg, color = "#2a2a4a", "#ccccff"
        else:
            bg, color = "#1e2a1e", "#ccffcc"
